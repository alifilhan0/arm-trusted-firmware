...
extern unsigned int gis_abnormal_boot;
...
void mt_log_setup(unsigned int start, unsigned int size, unsigned int aee_buf_size)
{
	...
	debug_print("cflag: 0x%x\n", p_atf_log_ctrl->info.atf_crash_flag);
    if(!gis_abnormal_boot || ATF_DUMP_DONE_MAGIC_NO == p_atf_log_ctrl->info.atf_crash_flag) {
		...
		if(dump_ret) {
			...
			if(ATF_LAST_LOG_MAGIC_NO == local_flag){
				...
				p_atf_log_ctrl->info.atf_crash_flag = ATF_CRASH_MAGIC_NO;
				debug_print("p_atf_log_ctrl->info.atf_crash_flag = ATF_CRASH_MAGIC_NO\n");
			}
		}
#if 0
		else{
...
void mt_log_secure_os_print(int c)
{
    ...
    if( c == '\n' )
    {
        ...
        printf("[%s]%s","uTos", mt_log_secure_os_buf);
#else
